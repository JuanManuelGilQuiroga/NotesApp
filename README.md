...
2. Instalacion de dependencias
 ```bash
   npm install
   ```
3. Ejecucion del frontend(terminal #1)
 ```bash
   npm run dev
   ```
4. Ejecucion del backend(terminal #2)
 ```bash
   npm run start
   ```
...
MONGO_URL=mongodb://admin:admin@junction.proxy.rlwy.net:13517/notesApp
...
| **Crear Nueva Versión de Nota**      | PUT            | `/notes/{id}/history` | Guarda una nueva versión de una nota. **(Sin interfaz gráfica)** |
